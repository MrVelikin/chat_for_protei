...
void init()
{
    ...
    (
        keywords::file_name = "Mylog_%N.log",                                        /*< file name pattern >*/
    ...
    {
        {
            ...
            std::cout << "[" << std::this_thread::get_id() << "] Поток запустился" << std::endl;
        }
        ...
        {
            std::lock_guard < std::mutex > lock(m);
            std::cout << "[" << std::this_thread::get_id() << "] Поток окончился" << std::endl;
        }
...
int main(int argc, char* argv[])
{
    ...
    BOOST_LOG_SEV(lg, debug) << "Start of program";
 try
    {
        if (argc < 2)
        {
            std::cout<< "Используйте:server <порт>\n";
            BOOST_LOG_SEV(lg, error) << "Неправильный формат записи "<< argc ;
            return 1;
        }
